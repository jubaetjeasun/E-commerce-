<?php
    ...
    if (isset($_REQUEST["submit-form"])) {
        ...
        $q = "select * from farmer where phone='$uname' && password=MD5('$password')";

        $_result = mysqli_query($conn, $q);
        $num = mysqli_num_rows($_result);

        if ($num == 1) {
            $query = "select * from farmer where phone='$uname' && password=MD5('$password') ";
            ...
            $login_error = "Sorry Phone/Password do not match";
